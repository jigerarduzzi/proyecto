...
def crearLey(bora,linesBora, inicio, fin, nLey):
    with open('Leyes/Bora'+bora.replace('seccion_primera','')+'_Ley'+str(nLey)+".txt", 'w') as l:
        while (inicio <= fin):
...
for bora in os.listdir('./BorasXML/'):
    borasxml.append(Path(os.path.join("./BorasXML/",bora)).stem)


for bora in borasxml:
    with open("./BorasXML/"+bora+'.xml', 'r') as f:
